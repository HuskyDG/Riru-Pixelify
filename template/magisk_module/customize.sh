...
if [ "$ABI32" == "armeabi-v7a" ]; then
    mv -f "$MODPATH/lib/armeabi-v7a" "$MODPATH/riru/lib"
    $IS64BIT && mv -f "$MODPATH/lib/arm64-v8a" "$MODPATH/riru/lib64"
else
   mv -f "$MODPATH/lib/x86" "$MODPATH/riru/lib"
    $IS64BIT && mv -f "$MODPATH/lib/x86_64" "$MODPATH/riru/lib64"
fi

if [ "$RIRU_API" == 0 ]; then
  ui_print "- Riru not installed"
  rm -rf "$MODPATH/riru"
elif [ "$RIRU_API" -lt 11 ]; then
...
elif [ -d "$MODPATH/riru" ]; then
    print ""
    print "- Riru core detected"
    print "  Do not enable Zygisk"
    print "  Overwise, Spoofing Google apps will not work"
    print ""
...
rm -rf $pix/apps_temp.txt
rm -rf $MODPATH/lib $MODPATH/riru.sh $MODPATH/addon
